...
def run_debug_mode(headers, all_data):
    ...
    if response:
        ...
        num_doubles = len(response) // 8                      
        logs = struct.unpack(f'{num_doubles}d', response)    
        
        with open("arduino_debug_log_v0.csv", "w", newline="") as f:
            writer = csv.writer(f)
            # Added new column headers to match R
            writer.writerow(["Direction", "Exp_Interval", "Start_Index", "End_Index", "Candidate_Index", "Max_CUSUM", "Order", "Cusum_before", "Cusum_after"])
            ...
            for i in range(0, len(logs), 9):
                ...
                max_cusum = logs[i+5]
                order = int(logs[i+6])
                cusum_before = logs[i+7] # New
                cusum_after = logs[i+8]  # New
                
                writer.writerow([direction, exp_interval, start_index, end_index, cand_index, f"{max_cusum:.7f}", order, f"{cusum_before:.7f}", f"{cusum_after:.7f}"])
                
        print(f"SUCCESS: Saved {(len(logs)//9)-1} log entries to arduino_debug_log.csv")
    else:
...
def main():
    ...
    headers, all_data = read_multi_column_csv("x_lin_2026.csv")            
